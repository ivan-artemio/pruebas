    ...
    {	
		...
		else
		{
			...
			try {
				...
				//Insertamos al profesor o director es su respectiva tabla
				if($post["role"] == 16 OR $post["role"] == 15){
					$idProf_dir = $this->editarNombreProf_o_direct($post);
		    	}else
		    		$idProf_dir = 0;
				...
				$random = rand(1000000000, mt_getrandmax());	
				$usuario = $this->_db->prepare("INSERT INTO da_usuarios (nombre, usuario, pass, email, role, estado, fecha, codigo, imagen, id_profesor) values(:nombre, :usuario, :password, :email, :role, :estado, :fecha, :codigo, :nombreImagen, :id_profesor)");
