...
class registroModel extends Model
{
    ...
    {
    	$random = rand(1000000000, mt_getrandmax());		
        $this->_db->prepare(
	...
	public function getRoles(){
		if($_SESSION['rol'] == 15){
			$roles = $this->_db->query(
					"SELECT id_role, role FROM da_roles WHERE id_role = 16 ORDER BY id_role DESC "
					);
        }else{
        	$roles = $this->_db->query(
    ...
    {	
    	/*if($post["role"] == 16){
    		$nombre = $this->getNombreProfe($post["id_profesor"]);
    		$id_profe = $post["id_profesor"];
    	}else if($post["role"] == 15){
    		$nombre = $this->getNombreDirector($post["id_director"]);
    		$id_profe = $post["id_director"];    		
    	}else{
    		$nombre = $post["nombre"];
    		$id_profe = 0;
    	}*/
		...
		{			
			try {
				...
				//modifico al profesor o director EN LA TABLA PROFESOREs
				//obtenemos el plantel
				if($_SESSION["rol"] == 15){
					$idPlantel = $this->getIdPlantelByIdUsuario($_SESSION['id_usuario']);					
				}else{		    				
					$idPlantel = $post["idPlantel"];
		    	}

		    	if($post["role"] == 16){ //  Si es profesor	
		    		$idProf_dir = $this->getIDProfesor($post["idUsuario"]);						    
					$this->modificarNombreProf_o_direct($post, $idProf_dir, $idPlantel);												
				}else
					$idProf_dir = 0;
		    	...
		    	/*if($post["role"] == 16){
		    		if($_SESSION["rol"] == 15){
						...
						$idPlantel = $this->getIdPlantelByIdUsuario($_SESSION['id_usuario']); 						
		    		}else if($_SESSION["rol"] == 1 OR $_SESSION["rol"] == 14){
		    			$idProf_dir = $this->getIDProfesor($post["idUsuario"]);
						$this->modificarNombreProf_o_direct($post, $idProf_dir);
						$idPlantel = $post["idPlantel"];
		    		}					
				...
				if(empty($post["pass"])){
					$usuario = $this->_db->prepare("UPDATE da_usuarios SET nombre=:nombre, usuario=:usuario, email=:email, imagen=:nombreImagen, id_plantel=:id_plantel WHERE id = :id LIMIT 1");	
					$res = $usuario->execute(
						array(                   				   
						':nombre' => $post["nombre"],  
				...
				//si hay contraseña
					...
					$usuario = $this->_db->prepare("UPDATE da_usuarios SET nombre=:nombre, usuario=:usuario, email=:email, pass=:password, imagen=:nombreImagen, id_plantel=:id_plantel WHERE id = :id LIMIT 1");	
					$res = $usuario->execute(
						array(                   				   
						':nombre' => $post["nombre"], 
						':usuario' => $post["usuario"],                                        
		...
		else
		{
			...
			try {
		    	...
		    	if($post["role"] == 16){ //  Si es profesor							    
					$idProf_dir = $this->editarNombreProf_o_direct($post, $idPlantel);												
				}else
					$idProf_dir = 0;
				...
				$random = rand(1000000000, mt_getrandmax());	
				$usuario = $this->_db->prepare("INSERT INTO da_usuarios (nombre, usuario, pass, email, role, estado, fecha, codigo, imagen, id_profesor, id_sistema, id_plantel) values(:nombre, :usuario, :password, :email, :role, :estado, :fecha, :codigo, :nombreImagen, :id_profesor, :id_sistema, :id_plantel)");
		...
		$jsondata = array();  
        if(!$post["idUsuario"] ){ //si user es 0
			$usuario = $this->verificarUsuario($post["usuario"]);			
			if(!$usuario){                               
	            $jsondata['response'] = 3;
	            $jsondata['mensajeValidacion'] = "Error al registrar el usuario";
	...
	public function editarNombreProf_o_direct($post, $idPlantel){                          
	...
	public function modificarNombreProf_o_direct($post, $idProf_dir, $idPlantel){                          
    ...
    public function getIdPlantelByIdUsuario($id_usuario)
    {    
        $res = $this->_db->query("SELECT usu.id_plantel FROM da_usuarios usu WHERE usu.id = $id_usuario LIMIT 1");        
        $row = $res->fetch();
        return $row[0];        
    }
    ...
    /*public function getIdPlantelByIdUsuario($idPlantel)
    {   
        $id = (int) $idPlantel;    
        $ficha = $this->_db->query("SELECT u.id_plantel FROM da_usuarios u WHERE u.id = $id LIMIT 1");       
        $row = $ficha->fetch();
        return $row[0];
    }*/
    ...
    {      
        ...
        if($rol == 16){ //rol profesor
            $res = $this->_db->prepare("SELECT usu.id_plantel FROM da_usuarios usu 
